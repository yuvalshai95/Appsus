...
function addGoogleBook(book) {
  const {title, authors, publishedDate, description, pageCount, categories, imageLinks, language} =
    book.volumeInfo;
  ...
  const bookToAdd = {
    ...
    listPrice: {
      ...
      currencyCode: Math.random() > 0.5 ? 'EUR' : 'USD',
      isOnSale: false,
    },
  };
...
function getGoogleBooksByName(name) {
...
function getPublishedTime(publishedDate) {
  ...
  if (now - publishedDate >= 10) {
    return 'Veteran Book';
  } else if (now - publishedDate < 1) {
    return 'New Book';
  } else {
    return 'Old Book';
  }
...
function _getFilteredBooks(books, filterBy) {
  ...
  return books.filter(
    book =>
      book.title.includes(name.toLowerCase()) &&
